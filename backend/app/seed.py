...
LUMBAR_ITEMS = [
    (
        ...
        "坐姿與站姿腰部伸展，每次 15 分鐘",
        "動作放慢，避免快速扭轉",
        "/videos/examples/lumbar-stretch.mp4",
        "示範坐姿與站姿兩種伸展，注意骨盆保持中立",
    ),
    (
        "核心橋式",
        "每週 3 次",
        ...
        "仰臥橋式 2 組，每組 10 次",
        "避免腰部過度上拱",
        "/videos/examples/core-bridge.mp4",
        "臀部抬起時停留 3 秒，注意呼吸節奏",
    ),
    (
        "貓牛式伸展",
...
SHOULDER_ITEMS = [
    (
        ...
        "每週 3 次",
        3,
        "身體前傾放鬆擺動患側手臂，每次 10 分鐘",
        "擺動幅度由小到大",
        "/videos/examples/shoulder-pendulum.mp4",
        "身體前傾約 90 度，手臂完全放鬆自然擺動",
    ),
    (
        "彈力帶外旋訓練",
        "每週 3 次",
        3,
        "手肘貼緊身體外旋 3 組，每組 12 次",
        "出現刺痛立即停止",
        "/videos/examples/band-external-rotation.mp4",
        "手肘夾緊毛巾，僅前臂旋轉",
    ),
    (
        "爬牆運動",
...
KNEE_ITEMS = [
    (
        ...
        "坐姿膝伸直 3 組，每組 10 次",
        "膝蓋不過度彎曲",
        "/videos/examples/quad-setting.mp4",
        "膝窩下壓毛巾捲，大腿前側用力",
    ),
    (
        "直腿抬高",
        ...
        "仰臥直腿抬高 3 組，每組 10 次",
        "腰部貼平地面",
        "/videos/examples/straight-leg-raise.mp4",
        "腳尖朝上，抬至 45 度停留 3 秒",
    ),
    ("靠牆半蹲", "每週 2 次", 2, "背靠牆緩慢下蹲至半蹲，每次 8 下", "膝蓋不超過腳尖", None, None),
]
NECK_ITEMS = [
    (
        "頸部伸展",
        ...
        "左右側彎與前後伸展，每次 10 分鐘",
        "避免快速轉頭",
        "/videos/examples/neck-stretch.mp4",
        "每個方向停留 15 秒，肩膀保持放鬆",
    ),
    ("肩胛收縮運動", "每週 3 次", 3, "雙肩向後夾緊 3 組，每組 12 次", "配合呼吸節奏", None, None),
]
BALANCE_ITEM = (
    "平衡訓練",
    "每週 2 次",
...
def _summary_text(score: float, item_name: str) -> str:
    ...
    if score >= 60:
        return (
            f"「{item_name}」動作穩定度不足，多個關節角度與範例落差明顯，建議對照範例影片修正姿勢。"
        )
...
def seed_demo(db: Session, staff: dict[str, User], patients: dict[str, Patient]) -> None:
    ...
    add_submission(
        db, pl05, n1, days_ago=1, hour=10, score=55, item_index=0, status="PENDING_REVIEW"
    )
    report(
        db,
        pl05,
    ...
    add_submission(
        db, pl12, n2, days_ago=1, hour=14, score=56, item_index=2, status="PENDING_REVIEW"
    )
    report(
        db,
        pl12,
    ...
    add_submission(db, pl17, n3, days_ago=0, hour=9, score=None, item_index=1, status="ANALYZING")
    ...
    add_submission(db, pl19, n3, days_ago=0, hour=7, score=None, item_index=0, status="ANALYZING")
