...
video_writer_plain = cv2.VideoWriter(f"output_4_video/{charactor[0]}_plain.mp4", fourcc, 30, (1920, 1080))
...
for vid_num in range(1):
    ...
    for index in range(len(patient1_result)):
        ...
        for frames in range(frames_show_count[1]):
            ...
            # 輸出僅含 4 個影片的版本 (2D/3D 導師+病患)
            top_left = cv2.resize(img3, (960, 540))
            bottom_left = cv2.resize(img4, (960, 540))
            top_right = cv2.resize(img5, (960, 540))
            bottom_right = cv2.resize(img6, (960, 540))
            plain_top = np.hstack((top_left, top_right))
            plain_bottom = np.hstack((bottom_left, bottom_right))
            plain_frame = np.vstack((plain_top, plain_bottom))
