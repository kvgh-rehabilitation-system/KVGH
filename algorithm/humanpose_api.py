...
def main_fun(charactor1, charactor2):
    ...
    # 繪製配對關係
    drawn_one_to_one = set()
    for (i, j, z) in zip(indices_x, indices_y_mapped, indices_y):
        # 判断是否是 filter_one_to_one 结果中的點
        if i in indices_x_one_to_one and i not in drawn_one_to_one and unique_index < len(unique_y_pairs_length):
    ...
    # 繪製配對關係
    drawn_one_to_one_left = set()
    for (i, j, z) in zip(indices_x_left, indices_y_mapped_left, indices_y_left):
        # 判断是否是 filter_one_to_one 结果中的點
        if i in indices_x_left_one_to_one and i not in drawn_one_to_one_left and unique_index < len(unique_y_pairs_left_length):
    ...
    # 繪製配對關係
    drawn_one_to_one_right = set()
    for (i, j, z) in zip(indices_x_right, indices_y_mapped_right, indices_y_right):
        # 判断是否是 filter_one_to_one 结果中的點
        if i in indices_x_right_one_to_one and i not in drawn_one_to_one_right and unique_index < len(unique_y_pairs_right_length):
            # 使用 unique_y_pairs_right 的值來繪製 Mentor 的 x, y 座標，避免當 filter_one_to_one 觸發跨區間配對方法時，patient frame 會配對到舊的 mentor 配對點，而不會成功跨區間配對
    ...
    for frame in result:
        if frame in list1:
            ...
            if unique_index_all < len(unique_y_pairs_length):
        ...
        if frame in list2:
            ...
            i = frame
            unique_index_left = list2.index(i)
            if unique_index_left < len(unique_y_pairs_left_length):
        ...
        if frame in list3:
            ...
            i = frame
            unique_index_right = list3.index(i)
            if unique_index_right < len(unique_y_pairs_right_length):
...
class _SeqFrameReader:
    """順序讀取器：持久持有一個 VideoCapture，往前循序讀到目標幀並快取當前幀。

    渲染主迴圈的幀索引（DTW 對齊後）大多單調非遞減且常有重複（Video Frozen），
    故：索引==上次→直接回快取；索引>上次→循序讀進；索引<上次（罕見回退）→重新 seek。
    取代每幀 open/seek/decode/close 的隨機存取，將解碼降為每支影片一次循序掃描。
    回傳 copy 以完全比照舊 getframe 語義（呼叫端可自由就地改圖）。
    """

    def __init__(self, vidname):
        self.vidname = vidname
        self.cap = cv2.VideoCapture(vidname)
        if not self.cap.isOpened():
            raise ValueError("Cannot open video file.")
        self.idx = -1          # 已讀到的 0-based 幀索引
        self.frame = None

    def get(self, frame_index):
        if frame_index != self.idx or self.frame is None:
            if frame_index < self.idx:
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, frame_index)
                ret, frame = self.cap.read()
                if not ret:
                    raise ValueError(f"Cannot read frame {frame_index}.")
                self.idx, self.frame = frame_index, frame
            else:
                while self.idx < frame_index:
                    ret, frame = self.cap.read()
                    if not ret:
                        raise ValueError(f"Cannot read frame {frame_index}.")
                    self.idx += 1
...
def getframe_cached(vidname, frame_index):
    """getframe 的高速替代：以檔名為鍵共用持久順序讀取器（語義與 getframe 相同）。"""
    reader = _frame_reader_cache.get(vidname)
    if reader is None:
        reader = _SeqFrameReader(vidname)
        _frame_reader_cache[vidname] = reader
    return reader.get(frame_index)


def _release_frame_readers():
    for reader in _frame_reader_cache.values():
        try:
            reader.release()
        except Exception:
            pass
    _frame_reader_cache.clear()
...
def imread_cached(path):
    """快取靜態圖，避免在每幀重複解碼同一張 JPG；回傳 copy 比照 cv2.imread 語義。"""
...
# 主程式執行參數（可依需求修改）
parser = argparse.ArgumentParser(description="動作相似度比對")
parser.add_argument("--charactor", nargs="+", default=["golf2"],
                    help="病人影片名稱列表（不含副檔名）")
parser.add_argument("--charactor1", default="golf2",
                    help="導師影片名稱（不含副檔名）")
args, _ = parser.parse_known_args()

charactor = args.charactor  # 病人影片名稱列表（不含副檔名）
charactor1 = args.charactor1  # 導師影片名稱（不含副檔名）
...
# 將 map 轉為 list，避免後續重複使用時被耗盡
mentor_HightlightFrame = list(map(int, mentor_HightlightFrame))
# 依病患影片名稱推斷視角（與 main_fun 邏輯一致）
if charactor[0] == "DPPatient":
    video_title = "Front"
elif charactor[0] == "DPPatientRight":
    video_title = "Right"
elif charactor[0] == "DPPatientLeft":
    video_title = "Left"
else:
    video_title = "PatientAngle"
# 只保留每個動作的相似度（similarity_history[index]）
action_similarity = []
if ptre[0]:
    for matched_frame in ptre[0]:
        # similarity_history 在產生影片時取的是當前 patient frame (start[1])，收斂到目標配對點前一幀
        frame_idx = max(matched_frame - 1, 0)
        if frame_idx < len(vid_collect_similarity[0]):
            action_similarity.append(float(vid_collect_similarity[0][frame_idx]))
        else:
            action_similarity.append(None)
# 整理階梯圖資料供前端使用
steps = list(map(int, ptre[0])) if ptre[0] else []
actions = [idx + 1 for idx in range(len(steps))]  # 1-based 動作序號，與繪圖一致
# 逐幀資訊：病患每一幀的索引與對應相似度，可供前端 hover 顯示
patient_frames = list(range(len(vid_collect_similarity[0]))) if vid_collect_similarity[0] else []
similarity_seq = [float(s) if s is not None else None for s in vid_collect_similarity[0]] if vid_collect_similarity[0] else []
# 針對每個動作，輸出導師/病患對齊幀的 16 維角度對照
angles_entries = []
for idx, step in enumerate(steps):
    mentor_frame_idx = mentor_HightlightFrame[idx] if idx < len(mentor_HightlightFrame) else None
    if mentor_frame_idx is None:
        continue
    if mentor_frame_idx >= len(mentorVideo_vectors_AngleMatrix) or step >= len(patientVideo_vectors_AngleMatrix[0]):
        continue
    mentor_angles = mentorVideo_vectors_AngleMatrix[mentor_frame_idx]
    patient_angles = patientVideo_vectors_AngleMatrix[0][step]
    angles_list = []
    for a_idx in range(16):
        angles_list.append({
            "id": f"a{a_idx+1}",
            "mentor": float(mentor_angles[a_idx]),
            "patient": float(patient_angles[a_idx]),
        })
    angles_entries.append({
        "action": idx + 1,
        "mentor_frame": int(mentor_frame_idx),
        "patient_frame": int(step),
        "angles": angles_list,
    })
all_frames_angles = []
patient_angles_all = patientVideo_vectors_AngleMatrix[0]
mentor_angles_all = mentorVideo_vectors_AngleMatrix
max_frames = max(len(patient_angles_all), len(mentor_angles_all))
for frame_idx in range(max_frames):
    angles = []
    for a_idx in range(16):
        mentor_val = float(mentor_angles_all[frame_idx][a_idx]) if frame_idx < len(mentor_angles_all) else None
        patient_val = float(patient_angles_all[frame_idx][a_idx]) if frame_idx < len(patient_angles_all) else None
        entry = {"id": f"a{a_idx+1}"}
        if mentor_val is not None:
            entry["mentor"] = mentor_val
        if patient_val is not None:
            entry["patient"] = patient_val
        angles.append(entry)
    all_frames_angles.append({
        "frame": int(frame_idx),
        "angles": angles,
    })
# 將結果序列寫成 JSON（分數檔獨立資料夾）
os.makedirs("scores_json", exist_ok=True)
# 將每個動作的對應幀與分數整理成列表，便於前端直接使用
matches = []
for idx, step in enumerate(steps):
    match = {
        "action": idx + 1,
        "patient_frame": int(step),
        "mentor_frame": int(mentor_HightlightFrame[idx]) if idx < len(mentor_HightlightFrame) else None,
        "similarity": action_similarity[idx] if idx < len(action_similarity) else None,
    }
...
stair_json = {
    ...
    "steps": steps,
    "actions": actions,
    "mentor_hlt": mentor_HightlightFrame,
...
angles_json = {
    ...
    "patient": charactor[0],
    "video_title": video_title,
    "angles": angles_entries,
    "all_frames": all_frames_angles,
}
# 分開存檔：score 檔案寫入 scores_json，階梯圖資料獨立檔案
with open(f"scores_json/{charactor[0]}.json", "w", encoding="utf-8") as f:
    json.dump(score_json, f, ensure_ascii=False, indent=2)
# 階梯圖 JSON 放在獨立資料夾
os.makedirs("stair_json", exist_ok=True)
with open(f"stair_json/{charactor[0]}.json", "w", encoding="utf-8") as f:
    json.dump(stair_json, f, ensure_ascii=False, indent=2)
# 角度對照 JSON 放在獨立資料夾
os.makedirs("angles_json", exist_ok=True)
with open(f"angles_json/{charactor[0]}.json", "w", encoding="utf-8") as f:
    json.dump(angles_json, f, ensure_ascii=False, indent=2)

# ---------------------------------------------------------------------------
# KVGH 復健系統儀表板格式輸出（analysis_json/<病患>.json）
#
# a1~a16 是相鄰肢段向量的 cosine 值，deg = degrees(arccos(clip(v,-1,1))) 即關節夾角。
# 其中 8 維對應儀表板的 8 個關節（AnalysisResult / MotionReplay 契約）：
#   a3=左肩 a4=左肘 a6=右肩 a7=右肘 a12=左髖 a13=左膝 a15=右髖 a16=右膝
# 其餘 8 維（頭頸/肩線/軀幹/骨盆）用於姿勢分數。
# ---------------------------------------------------------------------------
A_TO_JOINT = {
    "a3": "left_shoulder", "a4": "left_elbow",
    "a6": "right_shoulder", "a7": "right_elbow",
    "a12": "left_hip", "a13": "left_knee",
    "a15": "right_hip", "a16": "right_knee",
}
MOTION_JOINTS_ORDER = [
...
JOINT_TO_A = {joint: a_id for a_id, joint in A_TO_JOINT.items()}
JOINT_LABELS_ZH = {
...
TRUNK_A_IDS = ("a1", "a2", "a5", "a8", "a9", "a10", "a11", "a14")
HIGH_DEVIATION_DEG = 15.0  # 關節偏差超過此角度標記 HIGH


def _cos_to_deg(value):
...
# 各關節偏差：重點動作幀上 |導師角度 - 病患角度| 的平均（度）
_joint_dev_samples = {a_id: [] for a_id in A_TO_JOINT}
for _entry in angles_entries:
    for _a in _entry["angles"]:
        if _a["id"] in _joint_dev_samples:
            _joint_dev_samples[_a["id"]].append(
                abs(_cos_to_deg(_a["mentor"]) - _cos_to_deg(_a["patient"]))
            )

joint_deviations = []
_dev_values = []
for _joint in MOTION_JOINTS_ORDER:
    _samples = _joint_dev_samples[JOINT_TO_A[_joint]]
    _dev = float(np.mean(_samples)) if _samples else 0.0
    _dev_values.append(_dev)
    joint_deviations.append({
        "joint": _joint,
        "label": JOINT_LABELS_ZH[_joint],
        "deviation_deg": round(_dev, 1),
        "status": "HIGH" if _dev > HIGH_DEVIATION_DEG else "OK",
    })

# 總分：各重點動作相似度（cosine 0~1）平均 → 0-100
overall_score = _clip100(float(np.mean(action_similarity)) * 100) if action_similarity else 0.0

# 關節分數：平均偏差每 1 度扣 2 分（25 度 → 50 分）
joint_angle_score = _clip100(100.0 - 2.0 * float(np.mean(_dev_values))) if _dev_values else overall_score

# 穩定度：逐幀相似度的波動（std）越小越穩（std 0.20 → 0 分）
_valid_sim = [s for s in similarity_seq if s is not None]
stability_score = (
    _clip100(100.0 * (1.0 - float(np.std(_valid_sim)) / 0.20))
    if len(_valid_sim) > 1
    else overall_score
)

# 姿勢分數：軀幹相關 8 維在全程逐幀的平均角度偏差
_trunk_samples = []
for _frame in all_frames_angles:
    for _a in _frame["angles"]:
        if _a["id"] in TRUNK_A_IDS and "mentor" in _a and "patient" in _a:
            _trunk_samples.append(abs(_cos_to_deg(_a["mentor"]) - _cos_to_deg(_a["patient"])))
posture_score = (
    _clip100(100.0 - 2.0 * float(np.mean(_trunk_samples))) if _trunk_samples else overall_score
)

# motion_sequence：病患逐幀 8 關節角度（度），驅動前端 3D 人物重播
_patient_video_path = f"alphapose_output/{charactor[0]}.mp4"
_fps = 30.0
_cap = cv2.VideoCapture(_patient_video_path)
if _cap.isOpened():
...
_motion_frames = []
for _frame in all_frames_angles:
    _by_id = {_a["id"]: _a for _a in _frame["angles"]}
    _row = []
    for _joint in MOTION_JOINTS_ORDER:
        _a = _by_id.get(JOINT_TO_A[_joint])
        if _a is None or "patient" not in _a:
            _row = None
            break
        _row.append(round(_cos_to_deg(_a["patient"]), 1))
    if _row is not None:
        _motion_frames.append(_row)

motion_sequence = {
    "fps": round(float(_fps), 2),
    "joints": MOTION_JOINTS_ORDER,
    "frames": _motion_frames,
}

# 中文摘要
_high_joints = [d["label"] for d in joint_deviations if d["status"] == "HIGH"]
if overall_score >= 85:
    summary_text = "動作標準度高，關節軌跡與範例影片高度一致，請保持目前品質。"
elif overall_score >= 72:
    summary_text = "整體完成度良好，少數關節角度略有偏差，建議放慢動作節奏。"
elif overall_score >= 60:
    summary_text = "動作穩定度不足，部分關節角度與範例落差明顯，建議對照範例影片修正姿勢。"
else:
    summary_text = "動作偏差較大且不穩定，建議護理師確認影片並指導修正，必要時回報醫生。"
if _high_joints:
    summary_text += f"（偏差較大的關節：{'、'.join(_high_joints)}）"

analysis_payload = {
    "mentor": charactor1,
    "patient": charactor[0],
    "overall_score": round(overall_score, 1),
    "joint_angle_score": round(joint_angle_score, 1),
...
with open(f"analysis_json/{charactor[0]}.json", "w", encoding="utf-8") as f:
    json.dump(analysis_payload, f, ensure_ascii=False, indent=2)
print(f"analysis_json/{charactor[0]}.json written "
      f"(overall={overall_score:.1f}, joint={joint_angle_score:.1f}, "
      f"stability={stability_score:.1f}, posture={posture_score:.1f})")
...
def make_dtw_result_img(result, draw_index,draw_bool,mentor_HightlightFrame):
    ...
    tick_count = min(len(yyyy) - 1, len(mentor_HightlightFrame))
    yticks = range(0, tick_count, 1)  # 獲取 Y 軸刻度位置
    #ylabels = [f'α{str(i+1)}' for i in yticks]  # 創建對應的標籤
    ylabels = [i for i in mentor_HightlightFrame[:tick_count]]  # 創建對應的標籤
...
def resize_with_padding(img, target_size, pad_color=(255, 255, 255)):
    """等比例縮放並補齊邊框，避免畫面被拉伸。"""
...
def crop_to_content(img, white_threshold: int = 245, margin: int = 5):
    """裁掉 3D 畫面四周白邊，保留一點緣邊用於 plain 四宮格。"""
...
def compose_plain_quad(img3, img4, img5, img6, portrait_layout=True):
    """依影片方向排版 2D/3D 導師+病患，保持比例不裁切。"""
    if portrait_layout:
        size_2d = (640, 1080)
        size_3d = (640, 540)
        c1 = resize_with_padding(img3, size_2d)
        c2 = resize_with_padding(img4, size_2d)
        c3 = resize_with_padding(img5, size_3d)
        c4 = resize_with_padding(img6, size_3d)
        right_col = np.vstack((c3, c4))
        canvas = np.hstack((c1, c2, right_col))
    else:
        size_2d = (1280, 540)
        size_3d = (640, 540)
        c1 = resize_with_padding(img3, size_2d)
        ...
        c4 = resize_with_padding(img6, size_3d)
        right_col = np.vstack((c3, c4))
        left_col = np.vstack((c1, c2))
        canvas = np.hstack((left_col, right_col))
    return canvas

def compose_square_quad(img3, img4, img5, img6, cell_size=960):
    """四宮格正方形排版，維持影格比例以 padding 填補。"""
    c1 = resize_with_padding(img3, (cell_size, cell_size))
    c2 = resize_with_padding(img4, (cell_size, cell_size))
    c3 = resize_with_padding(img5, (cell_size, cell_size))
    c4 = resize_with_padding(img6, (cell_size, cell_size))
    top = np.hstack((c1, c3))
    bottom = np.hstack((c2, c4))
    return np.vstack((top, bottom))


def draw_plain_overlay(img, frame_id, label, highlight, cell_size=960):
    """先填充到單格尺寸，再以格子邊界定座標與比例繪製文字。"""
    cell = resize_with_padding(img, (cell_size, cell_size))
    h, w = cell.shape[:2]
    font = cv2.FONT_HERSHEY_SIMPLEX

    # 以裁切版參考座標 (1120x1080) 映射到 960x960 單格，確保四格一致
    ref_w, ref_h = 1120.0, 1080.0
    scale = w / ref_w

    scale_main = 2.0 * scale   # 對應裁切版字級 2
    scale_sub = 1.5 * scale    # 對應裁切版字級 1.5

    x_frame = int(10 / ref_w * w)   # 原始 (10, 50)
    y_frame = int(50 / ref_h * h)
    x_frozen = int(5 / ref_w * w)   # 原始 (5, 100)
    y_frozen = int(100 / ref_h * h)

    cv2.putText(cell, f'Frame: {frame_id}', (x_frame, y_frame), font, scale_main, (0, 0, 0), 2)
    cv2.putText(cell, 'Video Frozen', (x_frozen, y_frozen), font, scale_sub, (0, 0, 255), 2)

    base_label_x = 900 if label.lower().startswith('mentor') else 700
    label_x = int(base_label_x / ref_w * w)
    cv2.putText(cell, label, (label_x, y_frame), font, scale_main, (0, 0, 0), 2)

    circle_center = (int(350 / ref_w * w), int(90 / ref_h * h))
    circle_radius = max(6, int(15 * scale))
    if highlight:
        cv2.circle(cell, circle_center, circle_radius, (0, 255, 0), -1)
    else:
        cv2.circle(cell, circle_center, circle_radius, (255, 255, 255), 2)
...
video_writer = cv2.VideoWriter(f"output_video/{charactor[0]}.mp4", fourcc, 30, (2500, 1395))#930
os.makedirs("output_4_video", exist_ok=True)
PLAIN_CELL_SIZE = 960
video_writer_plain = cv2.VideoWriter(f"output_4_video/{charactor[0]}_plain.mp4", fourcc, 30, (PLAIN_CELL_SIZE*2, PLAIN_CELL_SIZE*2))
# 判斷 2D 影片方向（以導師 2D 影片第一幀為準），決定排版形狀
try:
    probe_frame = getframe(f"alphapose_output/{charactor1}.mp4", 0)
    portrait_layout = probe_frame.shape[0] >= probe_frame.shape[1]
except Exception:
...
for vid_num in range(1):
    ...
    for index in range(step_count):
        ...
        for frames in range(frames_show_count[1]):
            ...
            temp_move = 50
            temp = imread_cached(f"ppt/AoL-based.jpg")
            ...
            img_to_draw = 0
            ##########################################################################
            # 讀取原始影格（plain 版使用，不裁切，保留文字）
            raw_img3 = getframe_cached(f"alphapose_output/{charactor1}.mp4", start[0])
            raw_img4 = getframe_cached(f"alphapose_output/{charactor2}.mp4", start[1])
            raw_img5 = getframe_cached(f"motionbert_output/{charactor1}.mp4", start[0])
            raw_img6 = getframe_cached(f"motionbert_output/{charactor2}.mp4", start[1])

            # 在原始 2D 影格上補文字，並縮放到單格尺寸，供 plain 四宮格使用
            plain_img3 = draw_plain_overlay(raw_img3, start[0], 'Mentor', draw_tag[0] or frames == frames_show_count[1]-1, cell_size=PLAIN_CELL_SIZE)

            plain_img4 = draw_plain_overlay(raw_img4, start[1], 'convalescent', draw_tag[1] or frames == frames_show_count[1]-1, cell_size=PLAIN_CELL_SIZE)
            ...
            # 輸出僅含 4 個影片的版本 (2D/3D 導師+病患) 正方形四宮格，僅在此裁除 3D 白邊
            plain_3d_mentor = crop_to_content(raw_img5, white_threshold=247, margin=5)
            plain_3d_patient = crop_to_content(raw_img6, white_threshold=247, margin=5)
            plain_frame = compose_square_quad(plain_img3, plain_img4, plain_3d_mentor, plain_3d_patient, cell_size=PLAIN_CELL_SIZE)
            video_writer_plain.write(plain_frame)
            ...
            if frames == frames_show_count[1]-1:
                ...
                if SAVE_IMAGES:
                    ...
                    try:
                        ...
                        if index == len(patient1_result) - 1:
                            os.makedirs("mapping_png", exist_ok=True)
                            cv2.imwrite(f"mapping_png/{charactor2}_mapping.png", hlt_pic_history)
...
video_writer_plain.release()
_release_frame_readers()
#video_writer2.release()

# 將 mp4v 轉成 H.264 並覆蓋同檔名
src = Path(f"output_video/{charactor[0]}.mp4")
tmp = src.with_name(f"{src.stem}_tmp.mp4")
if src.exists():
    cmd = [
        "ffmpeg", "-y",
        "-i", str(src),
        ...
        "-crf", "23",
        "-preset", "medium",
        "-movflags", "+faststart",  # moov 前置：瀏覽器可邊播邊緩衝
        str(tmp),
    ]
    proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if proc.returncode != 0:
        print("ffmpeg transcode failed:", proc.stderr)
    else:
        try:
            tmp.replace(src)
        except Exception as e:
            print("replace video error:", e)

src_plain = Path(f"output_4_video/{charactor[0]}_plain.mp4")
tmp_plain = src_plain.with_name(f"{src_plain.stem}_tmp.mp4")
if src_plain.exists():
    cmd_plain = [
        "ffmpeg", "-y",
        "-i", str(src_plain),
    ...
    proc_plain = subprocess.run(cmd_plain, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if proc_plain.returncode != 0:
        print("ffmpeg transcode failed (plain video):", proc_plain.stderr)
    else:
        try:
            tmp_plain.replace(src_plain)
        except Exception as e:
            print("replace plain video error:", e)
else:
    print(f"source video not found: {src_plain}")
